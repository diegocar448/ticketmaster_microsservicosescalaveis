...
| 11 | `baseUrl` depreciado no tsconfig.json | `moduleResolution: NodeNext` ignora `baseUrl` na resolução; ele só dispara `TS5101`, e o TS pinado (5.9.x) nem aceita `ignoreDeprecations: "6.0"` (→ `TS5103`) | **Remover** `baseUrl` (e qualquer `ignoreDeprecations`) do `tsconfig.json` por serviço — manter só `outDir`, `rootDir`, `experimentalDecorators`, `emitDecoratorMetadata` |
